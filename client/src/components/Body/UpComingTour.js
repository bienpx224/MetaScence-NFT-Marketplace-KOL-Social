...
const UpComingTour = () => {
  ...
  const { contract_market, contract_nft, account, isLogin, balance } = useSelector((state) => ({
  ...
  const onClickBuy = async (tokenId) => {
    if (isLogin) {
      if (contract_market && contract_nft) {
        ...
        contract_market.methods.buyToken(tokenId).send({ from: account })
    ...
    if (listTicket && listTicket[1]) {
      if (listTicket[1].seller != account && balance && balance == 0) {
        return (
          ...
          <a className="btn-s uppercase btn btn-primary with-ico border-4" onClick={() => { onClickBuy(listTicket[1].tokenId) }}><i className="icon-ticket" />{listTicket[1].price} ETH</a>
          <a className="btn-s uppercase btn btn-primary with-ico" onClick={() => { onClickBuy(listTicket[1].tokenId) }}><i className="icon-cart" />Buy Ticket</a>
        </div>
                  ...
                  <div className="block-content ">
                    <div className="row">
                      ...
                      <div className="col-lg-4 col-md-4">
                        <h6 className="mb-0 opc-70 uppercase">Tokyo, Japan</h6>
